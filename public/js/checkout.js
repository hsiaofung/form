...
$(document).ready(function() {
	...
	$(".sectTitle").on("click", function() {
		...
		$openedSect.children(".sectContent").slideUp(300, function(){
			$("html, body").animate({ "scrollTop": $( thisSect ).offset().top - 100 }, 300 ); //100 is manual number
		});

		$( thisSect ).children(".sectContent").slideDown(300);
	...
	$(".goSectBtn").on("click", function() {
		...
		//if( checked this section is Valid ){
			...
			if( $chectout_sect.hasClass("chectout_pdts") ){
				$(".sidebar_forSticky .checkout_discountCode_wrap").fadeOut(300, function(){
			...
			if( $( goSect ).hasClass("chectout_pdts") ){
				$(".sidebar_forSticky .checkout_discountCode_wrap").fadeIn(300, function(){
					stickySidebar.updateSticky();
				});
			}

			$chectout_sect.children(".sectContent").slideUp(300, function(){
				$("html, body").animate({ "scrollTop": $( goSect ).offset().top - 100 }, 300 ); //100 is manual number, should check height dynmically in desktop/mobile
			});
	...
	function checkBag(){
		$(".chectout_pdts .shoppingBagNum").text( $(".chectout_pdts .pdt_wrap").length );
	...
	$(".deliverOption_btn").on("click", function() {
		...
		$(".deliverOption_btn").find(".icon").each( function( index ) {
	...
	$(".opened").children(".sectContent").slideDown(300); 

	//if logged in...
	//if ( customer already have saved address ){
		//$(".chectout_sect .delivery_details_wrap").show();
	//} else{
		$(".chectout_sect .delivery_newAddress_wrap").show();
	//}
